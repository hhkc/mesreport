...
<body>
	<div class="panel panel-default">
		<div class="panel-heading">在制品库存查询条件</div>
...
$('#mytab').bootstrapTable({
	method : 'get',
	url : "epireport/workInProgress",//请求路径
	...
	queryParams : function(params) {//上传服务器的参数
		var temp = {//如果是在服务器端实现分页，limit、offset这两个参数是必须的
				 instance_id:$('#search_task').val().trim(),
...
function formatstatus(value, row, index) {
	...
	if(value == "WAITING"){
		...
		var a = '<span style="color:#28a745">'+value+'</span>';
	}else if(value == "DISPATH"){
		var a = '<span style="color:#17a2b8">'+value+'</span>';
	}else if(value == "HOLD"){
		var a = '<span style="color:#ffc107">'+value+'</span>';
	}
	return a;
}
//删除、编辑操作
...
$('#search_btn').click(function() {
	...
	$('#mytab').bootstrapTable('refresh', {
		url : '<%=request.getContextPath()%>/epireport/workInProgress'				
	});
